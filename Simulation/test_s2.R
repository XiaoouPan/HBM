...
epsilon_p_seq = seq(0.15, 0.27, by = 0.03)
epsilon_mu_seq = seq(0.5, 0.9, by = 0.1)
...
weak1 = weak2 = weak3 = weak4 = matrix(0, 5, 5)
strong1 = strong2 = strong3 = strong4 = matrix(0, 5, 5)
...
for (m in 1:M) {
  ...
  for (i in 1:5) {
    ...
    for (j in 1:5) {
      ...
      weak = reject_prob | reject_acti
      weak1[i, j] = weak1[i, j] + weak[1]
      weak2[i, j] = weak2[i, j] + weak[2]
      weak3[i, j] = weak3[i, j] + weak[3]
      weak4[i, j] = weak4[i, j] + weak[4]
      strong = reject_prob & reject_acti
      strong1[i, j] = strong1[i, j] + strong[1]
      strong2[i, j] = strong2[i, j] + strong[2]
      strong3[i, j] = strong3[i, j] + strong[3]
      strong4[i, j] = strong4[i, j] + strong[4]
    }
  }
